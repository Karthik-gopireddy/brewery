...
app.post('/reviewdetails', (req, res) => {
  const { breweryId, reviewer, rating, description } = req.body;
  const query = 'INSERT INTO reviews (brewery_id, reviewer, rating, description) VALUES (?, ?, ?, ?)';
  connection_3.query(query, [breweryId, reviewer, rating, description], (err, result) => {
    if (err) throw err;
...
app.get('/reviewdetails/:breweryId', (req, res) => {
  const breweryId = req.params.breweryId;
  const query = 'SELECT * FROM reviews WHERE brewery_id = ?';
  connection_3.query(query, [breweryId], (err, result) => {
      if (err) throw err;
      res.json(result);
  });
});